...
date_range = st.sidebar.date_input("Select Date Range", date_default, min_value=df['date'].min(), max_value=df['date'].max())
...
# DAU / MAU / Stickiness
# -------------------------
# DAU: unique users per day (after filters)
dau_daily = (
    filtered_df.groupby('date')['user_id']
    .nunique()
    .rename('DAU')
    .reset_index()
)

# MAU: unique users per calendar month (after filters)
filtered_df['month'] = filtered_df['date'].values.astype('datetime64[M]')
mau_monthly = (
...
# Monthly stickiness = average daily DAU in month / MAU in that month
dau_daily_m = dau_daily.copy()
if not dau_daily_m.empty:
    dau_daily_m['month'] = dau_daily_m['date'].values.astype('datetime64[M]')
avg_dau_by_month = (
    dau_daily_m.groupby('month')['DAU'].mean().reset_index().rename(columns={'DAU': 'avg_DAU'})
) if not dau_daily_m.empty else pd.DataFrame(columns=['month','avg_DAU'])
